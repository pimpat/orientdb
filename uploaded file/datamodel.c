...
ReturnErr setDataNameByID(char* dataID, char* dataName, DTPacket* dtPacket){
    ...
    int ret = ORD_SendCommand(sql,dtPacket);
    if (ret!=0){
...
char* getDataNameByID(char* dataID, DTPacket* dtPacket){
    ...
    char* result = ORD_GetContent(sql,dtPacket);
    printf("result: %s\n",result);
    if(result == NULL){
        return NULL;
    }
    char *token, *dataName, *ptr;
...
ObjectBinary* getDataContentByID(char* dataID, DTPacket* dtPacket){
    ...
    printf("SQL: %s\n",sql);
    result = ORD_GetContent(sql,dtPacket);
    printf("\nresult: %s\n",result);
    if(result==NULL){
        ...
        printf("before: %s\n",result);
        char* res_tmp = replace(result,"\\'","'");
        char* res = replace(res_tmp,"\\\"","\"");
        printf("\n------------------------------------------------------------------------------------------------------------\n\n");
        printf("replace: %s\n",res);
        printf("\n------------------------------------------------------------------------------------------------------------\n\n");
...
ReturnErr setNewDataContent(Data* data, ObjectBinary* content){
    ...
    if(data->content->head==NULL && data->content->lastestCommon==NULL && data->content->current==NULL){
        ...
        char* s_plus = getDiff(dc0->fullContent->data,content->data);
        char* s_minus = getDiff(content->data,dc0->fullContent->data);
        printf("s_plus: %s\n",s_plus);
        printf("s_minus: %s\n",s_minus);
...
ReturnErr saveNewDataContentByID(char* dataID, char* new_content, DTPacket* dtPacket){
    ...
    char* s_plus = getDiff(obj->data,new_content);
    char* s_minus = getDiff(new_content,obj->data);
    printf("s_plus: %s\n",s_plus);
    printf("s_minus: %s\n",s_minus);
    
    freeObjBinary(obj);
    
    int minus_len = (int)strlen(s_minus);
    int plus_len = (int)strlen(s_plus);
    ...
    printf("before: %s\n",s_plus);
    char* res_plus_tmp = replace(s_plus,"'","\\'");
    char* res_plus = replace(res_plus_tmp,"\"","\\\"");
    printf("replace(plus): %s\n",res_plus);
    free(res_plus_tmp);
    free(s_plus);
    
    printf("before: %s\n",s_minus);
    char* res_minus_tmp = replace(s_minus,"'","\\'");
    char* res_minus = replace(res_minus_tmp,"\"","\\\"");
    printf("replace(minus): %s\n",res_minus);
    free(res_minus_tmp);
    free(s_minus);
    
    printf("before: %s\n",new_content);
    char* res_full_tmp = replace(new_content,"'","\\'");
    char* res_full = replace(res_full_tmp,"\"","\\\"");
    ...
    printf("SQL: %s\n",sql);
    ret = ORD_SendCommand(sql,dtPacket);
    ...
    printf("SQL: %s\n",sql);
    ret = _ORD_CreateVertex_C(sql,&nhead_cltid,&nhead_rid,dtPacket);
...
const char* createData(Text dataName, Schema* dataSchema, int dType, DTPacket* dtPacket){
    ...
    ret = _ORD_CreateVertex_C(sql,&dt_cltid,&dt_rid,dtPacket);
    if (ret!=0){
    ...
    ret = _ORD_CreateVertex_C(sql,&dh_cltid,&dh_rid,dtPacket);
    if (ret!=0){
    ...
    if(dType==_user){
        ...
        ret = _ORD_CreateVertex_C(sql,&dl_cltid,&dl_rid,dtPacket);
        if (ret!=0){
    ...
    //    printf("SQL: %s\n",sql);
    ret = _ORD_CreateVertex_C(sql,&dc_cltid,&dc_rid,dtPacket);
    if (ret!=0){
...
ReturnErr addData2Data(char* dataID, Data* data, int dType, DTPacket* dtPacket){
    ...
    for(dc=data->content->lastestCommon;dc!=NULL;dc=next_dc){
        ...
        if(dc->fullContent != NULL && dc->minusPatch == NULL && dc->plusPatch == NULL){
            ...
            printf("before: %s\n",dc->fullContent->data);
            char* result_tmp = replace(dc->fullContent->data,"'","\\'");
            char* result = replace(result_tmp,"\"","\\\"");
            printf("replace: %s\n",result);
            free(result_tmp);
            
            sprintf(sql,"CREATE VERTEX Datacontent set isDiff=%d, SHA256hashCode='%s', full_schemaCode=%d, full_byteCount=%d, full_data='%s'",dc->isDiff,dc->SHA256hashCode,dc->fullContent->schemaCode,dc->fullContent->byteCount,result);
            free(result);
        }
            ...
            printf("before: %s\n",dc->minusPatch->data);
            char* result_tmp = replace(dc->minusPatch->data,"'","\\'");
            char* result = replace(result_tmp,"\"","\\\"");
            printf("replace: %s\n",result);
            free(result_tmp);
            
            sprintf(sql,"CREATE VERTEX Datacontent set isDiff=%d, SHA256hashCode='%s', minus_schemaCode=%d, minus_byteCount=%d, minus_data='%s'",dc->isDiff,dc->SHA256hashCode,dc->minusPatch->schemaCode,dc->minusPatch->byteCount,result);
        }
        printf("SQL: %s\n",sql);
        ret = _ORD_CreateVertex_C(sql,&dc_cltid[i],&dc_rid[i],dtPacket);
        if (ret!=0){
...
Data** queryDataFromData(char* dataID, int dType, DTPacket* dtPacket){
    ...
    int i,j;
    char** result_rid = ORD_GetArrayDistinctRid(sql,dtPacket);
    if(result_rid==NULL){
    ...
    char** dup_rid = ORD_GetArrayDistinctRid(sql,dtPacket);
...
Data* queryDataByRid(char* rid, DTPacket* dtPacket){
    ...
    char* tmp_tok = strtok_r(result,"$",&ptr);
...
t_bool isObjectUnderData(char* dataID, char* objID, DTPacket* dtPacket){
    ...
    char* ptr;
    char* token = strtok_r(result,":",&ptr);
    token = strtok_r(NULL,"l",&ptr);
    printf("count: %s\n",token);
...
Edge** getAllEdgeForThisObject(char* dataID, DTPacket* dtPacket){
    ...
    printf("--------------------------------------------------[get edgeRid,Class from adjacent Node]\n");
    sprintf(sql,"select @rid,@class from (select expand(bothE()) from Data where dataID='%s') where @class<>'toDataHolder'",dataID);
    printf("SQL: %s\n",sql);
    char* result = ORD_GetContent(sql,dtPacket);
    printf("\nresult: %s\n",result);
    
    char *token, *ptr;
    char* tmp_res = strdup(result);
    char* tmp_tok = strtok_r(tmp_res,"$",&ptr);
    ...
    char **edge_str = (char**)malloc(sizeof(char*)*count);
    
    token = strtok_r(result,"$",&ptr);
    for(i=0;i<count;i++){
        edge_str[i] = strdup(token);
        token = strtok_r(NULL,"$",&ptr);
    }
    free(result);
    
    Edge** arr_edge = (Edge**)malloc(sizeof(Edge*)*(count+1));
    arr_edge[count]=NULL;
    for(i=0;i<count;i++){
        arr_edge[i] = (Edge*)malloc(sizeof(Edge));
        token = strtok_r(edge_str[i],":",&ptr);
        token = strtok_r(NULL,",",&ptr);
        arr_edge[i]->edgeRid = strdup(token);
        
        token = strtok_r(NULL,":",&ptr);
        token = strtok_r(NULL,",",&ptr);
        arr_edge[i]->edgeName = strdup(token);
...
Data* getObjectPointByEdge(char* edgeRid, DTPacket* dtPacket){
    ...
    //  Data
    printf("--------------------------------------------------[get_info_Data]\n");
    sprintf(sql,"select dataID,dataName,dataType,chatRoom from (select expand(inV()) from %s)",edgeRid);
    printf("SQL: %s\n",sql);
    char* result = ORD_GetContent(sql,dtPacket);
    ...
    //  DataHolder
    printf("--------------------------------------------------[get_info_DataHolder]\n");
    sprintf(sql,"SELECT @rid,versionKeeped from (select expand(out('toDataHolder')) from (select expand(inV()) from %s))",edgeRid);
    printf("SQL: %s\n",sql);
    result = ORD_GetContent(sql,dtPacket);
